...
pub mod integrate {
    ...
    impl RK45 {
        ...
        pub fn new(f: fn(f64, f64) -> f64, eps: f64) -> RK45 {
            RK45 { f, eps }
        }

        /// 
        pub fn step(&self, t: f64, y: f64, h: f64) -> Result<(f64, f64), String> {
            let k1 = (self.f)(t, y);
            let k2 = (self.f)(t + h / 4.0, y + h * k1 / 4.0);
            let k3 = (self.f)(t + 3.0 * h / 8.0, y + h * (3.0 * k1 / 32.0 + 9.0 * k2 / 32.0));
            let k4 = (self.f)(t + 12.0 * h / 13.0, y + h * (1932.0 * k1 / 2197.0 - 7200.0 * k2 / 2197.0 + 7296.0 * k3 / 2197.0));
            let k5 = (self.f)(t + h, y + h * (439.0 * k1 / 216.0 - 8.0 * k2 + 3680.0 * k3 / 513.0 - 845.0 * k4 / 4104.0));
            let k6 = (self.f)(t + h / 2.0, y + h * (-8.0 * k1 / 27.0 + 2.0 * k2 - 3544.0 * k3 / 2565.0 + 1859.0 * k4 / 4104.0 - 11.0 * k5 / 40.0));
            let y1 = y + h * (25.0 * k1 / 216.0 + 1408.0 * k3 / 2565.0 + 2197.0 * k4 / 4104.0 - k5 / 5.0);
            let y2 = y + h * (16.0 * k1 / 135.0 + 6656.0 * k3 / 12825.0 + 28561.0 * k4 / 56430.0 - 9.0 * k5 / 50.0 + 2.0 * k6 / 55.0);
            let truncation_error = (y1 - y2).abs();
        ...
        /// use assert_approx_eq::assert_approx_eq;
        /// let rk4_integrator = RK45::new(|t, y| y, 0.01);
        /// let y = rk4_integrator.ivp(0.0, 1.0, 1.0);
        /// assert_approx_eq!(y.unwrap(), 2.718281828459045, 1e-6);
        /// ```
        pub fn ivp(&self, t0: f64, t1: f64, y0: f64, h0: f64) -> Result<f64, String> {
            if t1 < t0 {
                return Err("t1 < t0".to_string());
            }
